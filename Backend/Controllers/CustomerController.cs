...
namespace WebDb.Controllers
{
    ...
    public class CustomerController : ControllerBase
    {
        ...
        public CustomerController(ICustomerRepo repository, ILogger logger)
        {
        ...
        public async Task<IActionResult> AddNewCustomer(Customer customer)
        {
            if (ModelState.IsValid)
            {
                ...
                return Ok();//CreatedAtAction("GetItem", new { customer.Id }, customer);
            }
            return new JsonResult("Somethign Went wrong") { StatusCode = 500 };
        }

        [HttpPut]
        public async Task<IActionResult> UpsertCustomer(Guid id, Customer customer)
        {
